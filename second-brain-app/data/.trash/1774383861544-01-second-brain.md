# Second Brain（個人知識庫）建置 Prompt

> 🆓 **免費社群內容**

## 📖 說明

Second Brain 是一個本機化的個人知識庫瀏覽與捕捉介面。它直接讀取你電腦上的 Markdown 筆記資料夾，提供快速捕捉想法、全文搜尋、每日回顧、AI 對筆記提問等功能。核心價值：**不用搬移資料**、**不需要資料庫**、**用最少成本把日常想法落地**。

## ⚙️ Pre-requisites

- **Node.js 18+**（建議 LTS 版本）
- **一個 Markdown 筆記資料夾**（你現有的筆記，用 PARA 或任何結構都行）
- **（選用）OpenAI API Key**（如果要用 Copilot 對筆記提問和語意搜尋功能）
...
### 資料夾結構
你不一定要用 PARA（Projects/Areas/Resources/Archive）結構，可以用任何你喜歡的方式組織筆記。只要是 Markdown 檔案就能讀取。
