...
1. **檔案樹瀏覽（Sidebar）**
   ...
   1.1 **資料夾重新命名**：跳出一個視窗，預設會顯示目前的名稱，反白選取，輸入新的名稱，按下確定後，重新命名資料夾。 ✅
   1.2 **資料夾移動**：跳出一個 dialog 視窗，顯示整個資料夾的樹狀結構，選擇目標資料夾，按下確定後，移動資料夾。 ✅
   

2. **Markdown 閱讀器（MarkdownViewer）**
   - 支援 GFM（GitHub Flavored Markdown） ✅
   - 程式碼語法高亮 ✅
   - 自動生成大綱面板（Outline Panel），從 H1-H6 標題提取
   - 內部連結跳轉（`[[檔名]]` 格式）
   - 響應式設計（桌面/平板/手機） ✅

3. **快速 Capture（Quick Capture）**
   - 介面頂部輸入框，貼上任何文字內容 ✅
   - 自動偵測 URL，若是網頁連結則抓取標題作為檔名
   - 可選擇歸類資料夾（預設 00-inbox/） ✅
   - 一鍵存成新 Markdown 檔案，時間戳自動命名 ✅
